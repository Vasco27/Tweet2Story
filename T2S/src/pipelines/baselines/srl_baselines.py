...
if __name__ == '__main__':
    ...
    save_all_dir = results_dir + "all_topics/"

    try:
        os.mkdir(save_all_dir)
        print(f"Creating directory {save_all_dir}")
    except FileExistsError:
        warnings.warn(f"Directory {save_all_dir} already exists. Not creating...")

    tweetir_data = pd.read_csv(path_to_data)
    topic_ids = tweetir_data["topic"].unique()

    total_iterations, iteration = topic_ids.shape[0], 1
    for topic in topic_ids:
        start_iter_time = time.time()
        print(f"Iteration {iteration} of {total_iterations}")
        topic_dir = save_all_dir + topic + "/"
        try:
            os.mkdir(topic_dir)
        except FileExistsError:
            warnings.warn(f"Directory for topic {topic} already exists.\n{topic_dir}")
        ...
        # Tweets
        coref_tweets, tweets_clusters_list, _ = coref_with_lemma(tweets_single_doc)
        ...
        # Save results
        pickle.dump(topic_results, open(topic_dir + "topic_srl.p", "wb"))
        pickle.dump(topic_verbs_lemma, open(topic_dir + "topic_verbs_lemma.p", "wb"))
        pickle.dump(topic_verbs, open(topic_dir + "topic_verbs.p", "wb"))
        ...
        # Save results
        pickle.dump(tweets_results, open(topic_dir + "tweets_srl.p", "wb"))
        pickle.dump(tweets_verbs_lemma, open(topic_dir + "tweets_verbs_lemma.p", "wb"))
        pickle.dump(tweets_verbs, open(topic_dir + "tweets_verbs.p", "wb"))
        ...
        pickle.dump(common_verbs, open(topic_dir + "common_verbs.p", "wb"))
        ...
        if global_count != 0:
            ...
            srl_global_metrics = {"rouge1_precision": 0,
                                  "rouge1_recall": 0,
                                  "rouge1_f1": 0}
        ...
        print("\nExporting to JSON...")
        with open(topic_dir + "srl_coref.json", "w", encoding="utf-8") as f:
            f.write(json.dumps(data_row, cls=MyEncoder, ensure_ascii=False, indent=4))

        end_iter_time = time.time()
        print(f"Iteration time - {round(end_iter_time - start_iter_time, 2)} seconds")
        iteration += 1
