...
if __name__ == '__main__':
    ...
    for filename in os.listdir(news_drs_dir):
        ...
        for sent in tweets:
            words = sent.split()
            n_words = len(words)
            edge1_cut = int(n_words*0.4)
            node_cut = int(n_words*0.6)

            edge1 = ' '.join(words[:edge1_cut])
            node = ' '.join(words[edge1_cut:node_cut])
            edge2 = ' '.join(words[node_cut:])

            actor1_id = "T"+str(actor_id)
            t_actors[actor1_id] = edge1
            actor_id += 1
            actor2_id = "T"+str(actor_id)
            t_actors[actor2_id] = edge2
            actor_id += 1

            kg_list.append({"edge1": actor1_id, "node": node, "edge2": actor2_id})
    ...
    with open(os.path.join(results_path, "random_evaluation.json"), "w+", encoding="utf-8") as f:
